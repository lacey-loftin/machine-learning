...
# Take a look at the Training data classe variables
...
The training data is now split into a training and testing set. The data is partitioned by the  classe variable, which is the varible we will be predicting. The data is split into 60% for training and 40% for testing.
```{r}
...
sam.err
```
This model is 99% accurate on the testing data partitioned from the training data and the out of sample error is 0.01%. 
```{r}
plot(modFit)
```
